...
func (uc *ReviewUsecase) ListReviewByUserID(ctx context.Context, userID int64, page, size int) ([]*model.ReviewInfo, error) {
	uc.log.WithContext(ctx).Debugf("[biz] ListReviewByUserID: userID:%d", userID)
	// 分页参数
	if page <= 0 {
		page = DefaultPage
	}
...
func (uc *ReviewUsecase) ListReviewByStoreID(ctx context.Context, storeID int64, page, size int) ([]*MyReviewInfo, error) {
	uc.log.WithContext(ctx).Debugf("[biz] ListReviewByStoreID: storeID:%d", storeID)
	// 分页参数
	if page <= 0 {
		page = DefaultPage
	}
...
// 这样才可以给 MyTime 加自定义方法
type MyTime time.Time

// ES数据类型是"2006-01-02 15:04:05"
// time.Time的格式是"2006-01-02T15:04:05Z07:00"
// 所以json.Unmarshal会失败,自定义 Unmarshal 解析成"2006-01-02 15:04:05"格式

// 自定义时间格式化
// MarshalJSON json.Unmarshal 的时候会自动调用这个方法
func (t *MyTime) UnmarshalJSON(data []byte) error {
	// data 是一个字符串，格式是 "\"2006-01-02 15:04:05"\"
	// 去掉前后的双引号
